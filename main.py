...
from models.movie import Cinema
...
#write_to_xml(university.as_xml(), fileName)
#write_to_json(university.as_json(), fileName)
#`write_to_xml(movie.as_xml(), fileName)
#`write_to_json(movie.as_json(), fileName)
#write_to_xml(bank.as_xml(), fileName)
#write_to_json(bank.as_json(), fileName)

retail = Retail(store="Bai Store", address="123, Lucena", section="food", product="meat")
university = University(name="CEU", department="CAMT", student="Lods")
movie = Cinema(name='Waltz Cinema',open="9AM", close="10pm", movie="Interstellar") 
bank = Bank(name="BisaBank", transaction="Deposit", amount="100", date="today")
politician = Politician(name="Mar", age="50", achievement="Traffic Enforcer")
